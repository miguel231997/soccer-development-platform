...
VALUES ('Silver Lake Soccer Academy', 'Silver Lake', 'CA');
...
FROM seasons s WHERE s.name = '2025-2026';
...
FROM clubs c WHERE c.name = 'Silver Lake Soccer Academy';
...
VALUES
  ('admin@silverlake.test',
   crypt('Admin1234!',  gen_salt('bf', 10)),
   'System', 'Admin', 'ADMIN', true),
  ('coach@silverlake.test',
   crypt('Coach1234!',  gen_salt('bf', 10)),
   'Marcus', 'Rivera', 'COACH', true),
  ('parent@silverlake.test',
   crypt('Parent1234!', gen_salt('bf', 10)),
   'Jennifer', 'Torres', 'PARENT', true);
...
-- Use code SLSA-COACH-2025 to register a new coach, SLSA-PARENT-2025 for a parent
-- ─────────────────────────────────────────
...
VALUES
  ('Carlos', 'Mendez',  '2015-04-12', 'ST',  'CF',  'RIGHT', 9,  true,  true),
  ('Diego',  'Alvarez', '2015-06-23', 'CM',  'CAM', 'RIGHT', 8,  true,  true),
  ('Ethan',  'Brooks',  '2015-02-18', 'LW',  'RW',  'LEFT',  11, true,  true),
  ('Noah',   'Kim',     '2015-09-05', 'CB',  'CDM', 'RIGHT', 5,  false, true),
  ('Liam',   'Chen',    '2015-11-30', 'GK',  NULL,  'RIGHT', 1,  false, true);
...
SELECT
  (SELECT id FROM teams        WHERE name = '2015 Boys Orange'),
  (SELECT id FROM seasons      WHERE name = '2025-2026'),
  (SELECT id FROM season_phases WHERE name = 'Fall 2025'),
  (SELECT id FROM competitions  WHERE name = 'EDP'),
  'FC Oakland',
...
SELECT
  (SELECT id FROM teams        WHERE name = '2015 Boys Orange'),
  (SELECT id FROM seasons      WHERE name = '2025-2026'),
  (SELECT id FROM season_phases WHERE name = 'Spring 2026'),
  (SELECT id FROM competitions  WHERE name = 'EDP'),
  'Bay United FC',
...
SELECT
  (SELECT m.id FROM matches m JOIN teams t ON m.team_id = t.id WHERE m.opponent = 'FC Oakland' AND t.name = '2015 Boys Orange'),
