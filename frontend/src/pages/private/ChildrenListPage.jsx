  ...
  return (
    <div className="max-w-3xl space-y-8">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-black tracking-tight text-mig-text">My Children</h1>
      ...
      {registeredName && (
        <div className="bg-mig-success/10 border border-mig-success/20 rounded-lg px-4 py-3 text-sm text-mig-success">
      ...
      {!cLoading && !cError && children?.length === 0 && pendingRequests.length === 0 && (
        <div className="bg-mig-surface border border-mig-border rounded-lg p-8 text-center space-y-2">
          <p className="text-mig-text font-medium">No children registered yet.</p>
          <p className="text-sm text-mig-dim">
      ...
      {children && children.length > 0 && (
        <section className="space-y-3">
          <h2 className="text-xs text-mig-dim font-semibold uppercase tracking-wider">Active Players</h2>
          <div className="grid gap-3">
            {children.map((child) => (
              <button
                key={child.id}
                onClick={() => navigate(`/parent/children/${child.id}`)}
                className="bg-mig-surface border border-mig-border rounded-lg p-4 flex items-center gap-4 hover:border-mig-orange/40 hover:shadow-sm transition text-left w-full"
              >
                <div className="w-12 h-12 rounded-full bg-mig-orange/20 flex items-center justify-center text-mig-orange font-bold text-lg shrink-0 overflow-hidden">
                  {child.profileImageUrl
                    ? <img src={child.profileImageUrl} alt="" className="w-full h-full object-cover" />
                    : `${child.firstName?.[0]}${child.lastName?.[0]}`}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-mig-text">
                    {child.firstName} {child.lastName}
                    {child.jerseyNumber != null && (
                      <span className="ml-2 text-xs font-mono bg-mig-orange text-white px-1.5 py-0.5 rounded">
                        #{child.jerseyNumber}
                      </span>
                    )}
                  </p>
                ...
                <span className="text-mig-dim text-lg">›</span>
              </button>
            ))}
          </div>
        </section>
      ...
      {!rLoading && requests && requests.length > 0 && (
        <section className="space-y-3">
          <h2 className="text-xs text-mig-dim font-semibold uppercase tracking-wider">Registration Requests</h2>
          <div className="bg-mig-surface border border-mig-border rounded-xl divide-y divide-mig-border">
            {[...requests].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map((req) => (
              <div key={req.id} className="px-4 py-3 flex items-center justify-between gap-4">
                <div>
                  <p className="font-medium text-mig-text">{req.firstName} {req.lastName}</p>
                  ...
                  {req.rejectionReason && (
                    <p className="text-xs text-mig-danger mt-0.5">Reason: {req.rejectionReason}</p>
                  )}
                </div>
                <span className={`text-xs px-2 py-0.5 rounded font-medium shrink-0 ${STATUS_STYLE[req.status] ?? 'bg-mig-card text-mig-dim border border-mig-border'}`}>
                  {req.status}
                </span>
              </div>
            ))}
